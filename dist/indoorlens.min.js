...
`;document.head.appendChild(Vt);w=0});var ee={};ie(ee,{IndoorLensWidget:()=>Kt,buildIndoorLensDOM:()=>te});function xe(){let e=window.IndoorLens.fromid,t=window.IndoorLens.toid,n=window.IndoorLens.wchr==="true",o=window.IndoorLens.isapi==="true",s=document.getElementById("from"),a=document.getElementById("to"),i=document.getElementById("wchr"),r=document.getElementById("walk"),d=document.getElementById("api");if(o?d.style.display="none":d.style.display="flex",n?(i.style.display="flex",r.style.display="none"):(i.style.display="none",r.style.display="flex"),e){let l=D(e);s.value=`${l.name} on ${q(B(l.groupId))}`,s.disabled=!0}if(t){let l=D(t);a.value=`${l.name} on ${q(B(l.groupId))}`,a.disabled=!0}if(e&&t)at({fromId:e,toId:t,WCHR:n,onResult:l=>{G(l)}});else{if(!t)return;let l=D(t),u=[];if(!l?.surfaceBranchKeys?.length)return;for(let m=0;m<l.surfaceBranchKeys.length;m++){let x=l.surfaceBranchKeys[m],L=R?.branches[x];if(!L)return;u.push(...L.nodes.map(z=>({x:z.position.x,y:z.position.z})))}if(!u.length)return;let h=B(l.groupId);tt(h),ht(u,J)}}function Zt(){if(document.getElementById("indoorlens\u2011css"))return;let e=document.createElement("style");e.id="indoorlens\u2011css",e.textContent=`
  /* ---------- page\u2011level layout ------------------------------------------------ */
